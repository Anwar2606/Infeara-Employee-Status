...
const fetchTotalTasksCount = async () => {
  try {
    ...
    setTotalTasksCount(totalCount); // Make sure setTotalTasksCount is defined in your component
  } catch (error) {
 ...
 const timeStrToDecimal = (timeStr) => {
  ...
  useEffect(() => {
    ...
    const fetchClickupStatus = async (dateStr) => {
      try {
        ...
        querySnapshot.forEach((doc) => {
          const data = doc.data();
          if (data.status === "In Progress") yes++;
          else if (data.status === "Completed") no++;
        });
    ...
    useEffect(() => {
      ...
      fetchTotalTasksCount(selectedDate);
  ...
  id: 1,
  title: "Tasks",
  imageUrl: "https://www.freeiconspng.com/thumbs/tasks-icon/tasks-icon-7.png",
        ...
        <div className="main-content">
          <div className="card-wrapper">
            {cardData.map((card) => (
              ...
              {card.id === 1 ? (
  <p><strong>Total Tasks:</strong> {card.totalTasks}</p>
) : card.id !== 4 ? (
  <>
